...
bool RollingScene::init() {
	...
	if (!Layer::init()) {
		return false;
	}//先要将父类的初始化函数执行一遍以此初始化父类中的数据

	visibleSize= Director::getInstance()->getVisibleSize();

	Vec2 origin = Director::getInstance()->getVisibleOrigin();

	SimpleAudioEngine::getInstance()->playBackgroundMusic("Faded.mp3",true);
	...
	auto layer_rolling_1 = Sprite::create("BG_YH.jpg");//修改需要的背景
	...
	auto layer_rolling_2 = Sprite::create("BG_YH.jpg");
	...
	Score_show = Label::create("hello","Arial",45);
	Score_show->setPosition(Vec2(666, 1566));
	Score_show->setColor(ccc3(255, 140, 0));
	Score_show->setString(*scoreboard);

	this->addChild(Score_show, 8);



	player = Player::creatWithPicture("player.png");
	player->setPosition(Vec2(visibleSize.width / 2, 0));
	this->addChild(player,1);

	lifebar = Lifebar::createLifebar("hahaha.png", 25);
	lifebar->PlaceLifebar(Point(10 ,visibleSize.height-10),this);

	Sprite*backN = Sprite::create("backBtn.png");
	Sprite*backS = Sprite::create("backBtnS.png");
	
	MenuItemSprite* backBtn = MenuItemSprite::create(backN, backS, CC_CALLBACK_1(RollingScene::backToStart, this));
	...
	MenuItemSprite* stopBtn = MenuItemSprite::create(stopN, stopS, CC_CALLBACK_1(RollingScene::stop, this));
	...
	MenuItemSprite* stopmusicBtn = MenuItemSprite::create(stopmusicN, stopmusicS, CC_CALLBACK_1(RollingScene::stopmusic, this));
	...
	schedule(schedule_selector(RollingScene::update), 0.1, kRepeatForever, 0);
	
	schedule(schedule_selector(RollingScene::updateEnemey), 2, 1, 0);
	schedule(schedule_selector(RollingScene::updateEnemey2), 0.5, 1, 4);
	schedule(schedule_selector(RollingScene::updateEnemey3), 8, 1, 8);
	schedule(schedule_selector(RollingScene::updateEnemey4), 0.5, 10, 10);
	schedule(schedule_selector(RollingScene::updateEnemey5), 8, kRepeatForever, 0);
	schedule(schedule_selector(RollingScene::updateEnemey6), 10, kRepeatForever, 0);

	schedule(schedule_selector(RollingScene::updateEnhancepack), 10, kRepeatForever, 0);
	schedule(schedule_selector(RollingScene::CheckHp), 0.1);
	return true;
...
void RollingScene::onEnter() {
	...
	contactListener->onContactBegin = [&](PhysicsContact &contact)
	{
		...
		if (spriteB->getTag() == FRIEND && spriteA->getTag() == ENHANCE_PACK)
		{
			if (!spriteA->isVisible() || !spriteB->isVisible())
			{
				return false;
			...
			this->unschedule(schedule_selector(RollingScene::update_set_player_normal));
			this->schedule(schedule_selector(RollingScene::update_set_player_normal), 3, 1, 0);
...
void RollingScene::update(float dt) {
	checkmove();
}

void RollingScene::checkmove()
{
...
void RollingScene::updateEnemey(float dt)
{
...
void RollingScene::updateEnemey2(float dt)
{
...
void RollingScene::updateEnemey3(float dt)
{
...
void RollingScene::updateEnemey4(float dt)
{
...
void RollingScene::updateEnemey5(float dt)
{
...
void RollingScene::updateEnemey6(float dt)
{//碉堡似的飞机（打不死且与屏幕同步滚动）
	auto visibleSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();
...
void RollingScene::updateEnhancepack(float dt)
{
...
void RollingScene::update_set_player_normal(float dt)
{
	player_bullet_type = NORMAL_BULLET;
}

void RollingScene::backToStart(Ref* object)
{
...
void RollingScene::stop(Ref* object)
{
	...
	MenuItemSprite* continueBtn = MenuItemSprite::create(playN, playS, CC_CALLBACK_1(RollingScene::ContinueGame, this));
...
void RollingScene::ContinueGame(Ref* object)
{
	Vector<Node*>vec = getChildren();
	for (auto i : vec)
		i->resume();
...
void RollingScene::CheckHp(float time)
{
	...
	if ((*scoreboard)[6] == '1')
	{
		Director::getInstance()->replaceScene(Chapter2::creatChapter2(lifebar->Lifenum,scoreboard));
	}
}

void RollingScene::stopmusic(Ref* object)
{
